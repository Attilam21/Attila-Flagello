...
` },
      ...
      { inlineData: { mimeType: "text/uri-list", data: Buffer.from(`https://storage.googleapis.com/${bucket}/${encodeURIComponent(storagePath)}`).toString("base64") } },
      { text: `RAW_TEXT:\n${rawText}` },
      { text: `BASE_FIELDS:\n${JSON.stringify(baseFields)}` }
    ];

    let geminiFields = baseFields;
    let meta = { detectedLanguage: "it", confidence: needsReviewBase ? 0.6 : 0.8 };
    try {
      const resp = await gemini.generateContent({ contents: [{ role: "user", parts }], generationConfig: { temperature: 0.2, maxOutputTokens: 1200 } });
    ...
    const ocrDoc: OcrDoc = {
      uid,
      type,
      source: { storagePath, downloadURL: `https://storage.googleapis.com/${bucket}/${encodeURIComponent(storagePath)}` },
      vision: { engine: "cloud-vision.documentTextDetection", rawText, blocks, langHints: ["it"] },
      fields: geminiFields,
      meta: { ...meta, matchId },
      status: { parsed: true, needsReview: needsReviewBase || (meta.confidence ?? 0) < 0.75, errors: [] },
      createdAt: new Date(),
      updatedAt: new Date()
    };
    ...
    if (type === "ROSTER") {
      await db.doc(`users/${uid}/roster`).set({ ...geminiFields, _updatedAt: new Date() }, { merge: true });
    } else if (type === "MATCH_STATS") {
      await base.collection("stats").doc("main").set({ ...geminiFields, _updatedAt: new Date() }, { merge: true });
    } else if (type === "VOTES") {
      await base.collection("votes").doc("main").set({ ...geminiFields, _updatedAt: new Date() }, { merge: true });
    } else if (type === "HEATMAP") {
      await base.collection("heatmap").doc("main").set({ ...geminiFields, _updatedAt: new Date() }, { merge: true });
    } else if (type === "OPPONENT_FORMATION") {
      await db.doc(`users/${uid}/opponent/${matchId}`).set({ ...geminiFields, _updatedAt: new Date() }, { merge: true });
    }
  });
