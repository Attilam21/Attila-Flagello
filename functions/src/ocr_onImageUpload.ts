...
const MAX_CONSEC_ERRORS = parseInt(
  process.env.VERTEX_GEMINI_MAX_CONSEC_ERRORS || '5'
);
const CIRCUIT_COOLDOWN_MIN = parseInt(
  process.env.VERTEX_GEMINI_CIRCUIT_COOLDOWN_MIN || '15'
);
let gemini: ReturnType<VertexAI['getGenerativeModel']> | null = null;
const getGemini = () => {
...
async function checkAndIncreaseUsage(uid: string): Promise<{
  allowed: boolean;
  reason?: string;
}> {
...
async function recordGeminiError(uid: string): Promise<void> {
  try {
    const dateKey = getDateKey();
    const usageRef = db.doc(`users/${uid}/usage/gemini_${dateKey}`);
    await db.runTransaction(async tx => {
      ...
      const updates: any = { errors, lastErrorAt: now };
