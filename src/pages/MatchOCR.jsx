...
const MatchOCR = ({ user }) => {
  ...
  const handleHeatmapManualSubmit = async e => {
    ...
    try {
      ...
      showSuccessMessage('heatmap', '✅ Mappa di calore salvata con successo!');
      console.log('✅ Heatmap manuale salvata');
      
      // Reset form completo
      setHeatmapManualMode(false);
      setHeatmapData({
        zones: [
          { name: 'Centrale', percentage: 0 },
          { name: 'Sinistra', percentage: 0 },
      ...
      showErrorMessage('heatmap', '❌ Errore durante il salvataggio. Riprova.');
      
      // Fallback: Firebase down
      if (error.code === 'unavailable') {
        setFirebaseStatus('disconnected');
        showErrorMessage('heatmap', '⚠️ Firebase non disponibile. I dati verranno salvati localmente.');
      }
    }
  };
