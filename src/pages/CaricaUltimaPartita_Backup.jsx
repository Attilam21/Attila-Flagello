...
const CaricaUltimaPartita = ({ onPageChange }) => {
  ...
  const handleProcessOCR = async () => {
    ...
    if (uploadedCount < 4) {
      console.warn('⚠️ Carica tutte e 4 le immagini prima di procedere');
      alert('Carica tutte e 4 le immagini prima di procedere');
      return;
    }

    if (!auth.currentUser) {
      console.warn('⚠️ Devi essere loggato per elaborare le immagini');
    ...
    try {
      ...
      // BYPASS CORS: Vai direttamente ai dati demo
      console.log('🔄 BYPASS CORS: Usando dati demo direttamente...');
      ...
      console.log('📊 Dati salvati in Firestore e Dashboard aggiornata');
    } catch (error) {
      ...
      if (error.message.includes('Generative Language API non abilitata')) {
        console.error('❌ Generative Language API non abilitata');
        alert(
