...
export const searchPlayers = (query, filters = {}) => {
  ...
  if (filters.minRating) {
    results = results.filter(
      player => player.overall >= parseInt(filters.minRating)
    );
  }

  if (filters.maxRating) {
    results = results.filter(
      player => player.overall <= parseInt(filters.maxRating)
    );
  }

  if (filters.club && filters.club !== 'all') {
    results = results.filter(player => player.club === filters.club);
  }
...
export const getFilterOptions = () => {
  ...
  const nationalities = [
    ...new Set(database.map(p => p.nationality).filter(Boolean)),
  ];
