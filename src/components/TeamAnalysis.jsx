...
const TeamAnalysis = ({ players, formation, showDetails = false }) => {
  ...
  const analyzeTeam = async () => {
    ...
    try {
      ...
      const playerAnalyses = players.map(player => {
        ...
        return aiProfilingService.profilePlayer(player, assignedPosition, teamContext);
      });
      ...
      const teamSuggestions = generateTeamSuggestions(playerAnalyses, teamStats);
  ...
  const calculateTeamStats = (playerAnalyses) => {
    const totalRating = playerAnalyses.reduce((sum, analysis) => sum + analysis.overallRating, 0);
    const averageRating = Math.round(totalRating / playerAnalyses.length);

    const roleCompatibility = playerAnalyses.reduce((sum, analysis) => 
      sum + analysis.roleCompatibility.compatibility, 0) / playerAnalyses.length;

    const buildEfficiency = playerAnalyses.reduce((sum, analysis) => 
      sum + analysis.buildAnalysis.compatibility, 0) / playerAnalyses.length;

    const boosterEfficiency = playerAnalyses.reduce((sum, analysis) => 
      sum + analysis.boosterAnalysis.efficiency, 0) / playerAnalyses.length;

    const teamSynergy = playerAnalyses.reduce((sum, analysis) => 
      sum + analysis.teamSynergy.synergy, 0) / playerAnalyses.length;
    ...
    <div style={styles.container}>
      ...
      <div style={styles.teamStats}>
        <div style={styles.statCard}>
          <div style={{ ...styles.statValue, color: getRatingColor(teamAnalysis.teamStats.averageRating) }}>
        ...
        <div style={styles.statCard}>
          <div style={{ ...styles.statValue, color: getRatingColor(teamAnalysis.teamStats.roleCompatibility) }}>
        ...
        <div style={styles.statCard}>
          <div style={{ ...styles.statValue, color: getRatingColor(teamAnalysis.teamStats.buildEfficiency) }}>
        ...
        <div style={styles.statCard}>
          <div style={{ ...styles.statValue, color: getRatingColor(teamAnalysis.teamStats.teamSynergy) }}>
      ...
      {teamAnalysis.teamSuggestions.length > 0 && (
        <div style={styles.teamSuggestions}>
          <h3 style={{ color: '#E5E7EB', marginBottom: '1rem' }}>💡 Suggerimenti AI Squadra</h3>
