...
const FormationBuilder = ({ players, onSave, onReset }) => {
  ...
  const formations = [
    {
      name: '4-3-3',
      description: 'Formazione offensiva equilibrata',
      positions: 11,
    },
    {
      name: '4-4-2',
      description: 'Formazione classica bilanciata',
      positions: 11,
    },
    { name: '3-5-2', description: 'Formazione con 3 difensori', positions: 11 },
    {
      name: '4-2-3-1',
      description: 'Formazione moderna con trequartista',
      positions: 11,
    },
  ...
  const calculateFormationStats = () => {
    ...
    stats.overallRating = Math.round(
      positions.reduce((sum, player) => sum + (player.rating || 0), 0) /
        positions.length
    );
    ...
    // Calculate balance
    const attackPlayers = positions.filter(p =>
      ['LW', 'RW', 'ST', 'ST1', 'ST2'].includes(p.position)
    );
    const midfieldPlayers = positions.filter(p =>
      ['CM', 'CM1', 'CM2', 'CDM', 'LM', 'RM'].includes(p.position)
    );
    const defensePlayers = positions.filter(p =>
      ['GK', 'LB', 'RB', 'CB', 'CB1', 'CB2', 'CB3'].includes(p.position)
    );
      ...
      <div style={styles.mainContent}>
        ...
        <div style={styles.fieldContainer}>
          ...
          <div style={styles.field}>
            ...
            {/* Drop Zones */}
            {Object.entries(positionMap[formation] || {}).map(
              ([position, coords]) => (
                <div
                  key={position}
                  style={{
          ...
          <div style={styles.statsGrid}>
            ...
            <div style={styles.statCard}>
              <div style={styles.statValue}>
                {Object.keys(formationData).length}/11
              </div>
              ...
              >
                <div style={styles.playerInfo}>
                  ...
                  <div style={styles.playerDetails}>
                    ...
                    <div style={styles.playerPositionText}>
                      {player.position}
                    </div>
                  </div>
                </div>
