...
class AdvancedOCRService {
  ...
  async processImageWithTesseract(imageFile) {
    ...
    try {
      ...
      console.log('✅ eFootball OCR completed:', {
        imageType,
    ...
    for (const line of lines) {
      ...
      if (statMatch) {
        ...
        if (statName.includes('shooting') || statName.includes('tiro')) {
          ...
          statName.includes('passing') ||
          statName.includes('passaggio')
        ) {
          stats.passing = value;
        } else if (
          statName.includes('dribbling') ||
          statName.includes('dribbling')
        ) {
          stats.dribbling = value;
        } else if (
          statName.includes('defending') ||
          statName.includes('difesa')
        ) {
          stats.defending = value;
        } else if (
          statName.includes('physical') ||
          statName.includes('fisico')
        ) {
          stats.physical = value;
        } else if (
          statName.includes('speed') ||
          statName.includes('velocità')
        ) {
          stats.speed = value;
        }
      }
    ...
    try {
      ...
      switch (imageType) {
        ...
        default:
          result = {
            type: 'unknown',
            data: 'Immagine eFootball non riconosciuta',
          };
      }
