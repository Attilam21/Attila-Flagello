...
class AIProfilingService {
  constructor() {
    this.positionMap = {
      ...
      'CB': { name: 'Difensore Centrale', category: 'defender', priority: 'defensive' },
      ...
      'CAM': { name: 'Trequartista', category: 'midfielder', priority: 'offensive' },
      'LM': { name: 'Centrocampista Sinistro', category: 'midfielder', priority: 'mixed' },
      'RM': { name: 'Centrocampista Destro', category: 'midfielder', priority: 'mixed' },
    ...
    this.buildTypes = {
      'Finalizzatore': { focus: 'shooting', style: 'offensive', compatibility: ['ST', 'CF', 'LW', 'RW'] },
      'Regista': { focus: 'passing', style: 'creative', compatibility: ['CAM', 'CM', 'CDM'] },
      'Box-to-Box': { focus: 'physical', style: 'balanced', compatibility: ['CM', 'CDM', 'CAM'] },
      'Marcatore': { focus: 'defending', style: 'defensive', compatibility: ['CB', 'CDM', 'LB', 'RB'] },
      'Trequartista': { focus: 'dribbling', style: 'creative', compatibility: ['CAM', 'LW', 'RW'] },
      'Ala': { focus: 'speed', style: 'offensive', compatibility: ['LW', 'RW', 'LM', 'RM'] },
      'Difensore': { focus: 'defending', style: 'defensive', compatibility: ['CB', 'LB', 'RB'] },
      'Portiere': { focus: 'goalkeeping', style: 'defensive', compatibility: ['GK'] }
    };

    this.boosterTypes = {
      'Tiro': { stats: ['shooting'], positions: ['ST', 'CF', 'LW', 'RW', 'CAM'] },
      'Passaggio': { stats: ['passing'], positions: ['CM', 'CAM', 'CDM', 'LB', 'RB'] },
      'Dribbling': { stats: ['dribbling'], positions: ['LW', 'RW', 'CAM', 'ST', 'CF'] },
  ...
  profilePlayer(player, assignedPosition, teamContext = {}) {
    ...
    const analysis = {
      ...
      roleCompatibility: this.analyzeRoleCompatibility(player, assignedPosition),
  ...
  analyzeRoleCompatibility(player, assignedPosition) {
    ...
    if (!naturalPos || !assignedPos) {
      return { compatibility: 0, penalty: 0, reason: 'Posizione non riconosciuta' };
    }
    ...
    if (naturalPos.category === assignedPos.category) {
      return { compatibility: 80, penalty: 20, reason: 'Stessa categoria, ruolo diverso' };
    }
    ...
    if (adjacentCategories[naturalPos.category]?.includes(assignedPos.category)) {
  ...
  analyzeBuild(player, assignedPosition) {
    ...
    if (!buildInfo) {
      return { compatibility: 50, penalty: 0, reason: 'Build non riconosciuta' };
    }
  ...
  analyzeBoosters(player, assignedPosition) {
    if (!player.boosters || player.boosters.length === 0) {
      return { efficiency: 50, waste: 0, suggestions: ['Nessun booster attivo'] };
    }
    ...
    player.boosters.forEach(booster => {
      ...
      const isStatRelevant = this.isStatRelevantForPosition(boosterInfo.stats[0], assignedPosition);

      if (isRelevant && isStatRelevant) {
        totalEfficiency += 100;
        suggestions.push(`✅ Booster ${booster.type} ottimale per ${assignedPosition}`);
      } else if (isRelevant) {
        totalEfficiency += 70;
        suggestions.push(`⚠️ Booster ${booster.type} utile ma non prioritario`);
      } else {
        wasteCount++;
        suggestions.push(`❌ Booster ${booster.type} sprecato in ${assignedPosition}`);
      }
    });
  ...
  analyzeSkills(player, assignedPosition) {
    if (!player.skills || player.skills.length === 0) {
      return { utilization: 50, unused: 0, suggestions: ['Nessuna abilità rilevata'] };
    }
    ...
    const utilization = player.skills.length > 0 ? 
      Math.max(10, Math.round((utilizedSkills / player.skills.length) * 100)) : 50;
  ...
  analyzeTeamSynergy(player, teamContext) {
    if (!teamContext.players || teamContext.players.length === 0) {
      return { synergy: 50, connections: 0, suggestions: ['Contesto squadra non disponibile'] };
    }
    ...
    nearbyPlayers.forEach(nearbyPlayer => {
      ...
      if (connection.strength > 0) {
        ...
        suggestions.push(`🤝 Buona sinergia con ${nearbyPlayer.name} (${connection.reason})`);
      }
    });
  ...
  calculateOverallRating(analysis) {
    ...
    let rating = 0;
    rating += analysis.roleCompatibility.compatibility * weights.roleCompatibility;
  ...
  generateSuggestions(analysis) {
    ...
    if (analysis.skillsAnalysis.unused > 0) {
      suggestions.push({
        ...
        action: 'Considera di cambiare posizione per sfruttare tutte le abilità'
      });
    }
